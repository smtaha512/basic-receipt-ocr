...
def item_to_dict(item: list[str]) -> Item:
  ...
  index_of_quantity_and_price = [(match.start()) for match in re.finditer('\d+,\d+.EUR.\d', item[0])]
  
  name = item[0] if len(index_of_quantity_and_price) < 1 else item[0][:index_of_quantity_and_price[0] - 1]
  
  price_per_item = total
  quantity = 1
  
  if len(index_of_quantity_and_price) > 0:
   [price_per_item, quantity] = (item[0][index_of_quantity_and_price[0]:]).split('EUR')
...
def parse_receipt(receipt_content: list[str]) -> ParsedReceipt:
  ...
  index_of_last_item_price = receipt_content.index([i for i in receipt_content if re.findall("Posten:", i)][0]) - 1
  ...
  transaction_end_at: str = [item for _, item in enumerate(receipt_content) if re.search('\d{2}\.\d{2}\.\d{2}', item)][0]
  
  parsed_receipt: ParsedReceipt = { "items": [*mapped_items], "store_name": "Edeka", "date": datetime.strptime(transaction_end_at, '%d.%m.%Y').date() }
